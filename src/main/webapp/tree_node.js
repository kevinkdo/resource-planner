const TreeNode = React.createClass({
  getFill() {
    if (!this.props.can_view) {
      return "none";
    } else if (this.props.is_selected) {
      return '#5cb85c';
    } else {
  ...
  render() {
    ...
    var marker = <circle r="8" fill={this.getFill()} cx={this.props.x+8} cy={this.props.y+8} onMouseDown={this.handleMouseDown} onMouseUp={this.handleMouseUp}/>;
    ...
    var ellipse = <ellipse cx={this.props.x+8} cy={this.props.y+8} rx={9} ry={7} fill={this.props.can_view ? "none": "purple"} />;
    var text = <text className="nodelabel nopointer noselect" x={this.props.x + 20} y={this.props.y + 13}>{this.props.name}</text>;
    var deleteButton = <text className="deleteButton noselect" x={this.props.x - 15} y={this.props.y + 13} onClick={this.handleDeleteClick}>{this.props.is_selected ? "X" : ""}</text>;
    var editButton = <text className="editButton noselect" x={this.props.x - 45} y={this.props.y + 13} onClick={this.handleEditClick}>{this.props.is_selected ? "edit" : ""}</text>;
    return <g className={me_is_dragging ? "draggable nopointer" : "draggable yespointer"}>{marker}{circle}{ellipse}{deleteButton}{editButton}{text}</g>;
  }
});
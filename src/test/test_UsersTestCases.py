...
class UsersTestCases(unittest.TestCase):
  ...
  def test_CreateValidUserAsAdmin(self): #not sure how to check if admin
      ...
      r = requests.post(self.baseUrl + self.userUrl, data = json.dumps(newUser), headers = self.headers, verify = False)
      decoded = r.json()
  ...
  def test_CreateValidUserNotAsAdmin(self):
      ...
      r = requests.post(self.baseUrl + self.registerUrl, data = json.dumps(validUser), headers = self.headers, verify = False)
      r = requests.post(self.baseUrl + self.loginUrl, data = json.dumps(validUser), headers = self.headers, verify = False)
      ...
      r = requests.post(self.baseUrl + self.userUrl, data = json.dumps(newUser), headers = nonAdminHeader, verify = False)
      decoded = r.json()
  ...
  def test_GetUserWithValidID(self):
      r = requests.get(self.baseUrl + self.userUrl + '/1', headers = self.headers, verify = False)
      decoded = r.json()
      assert decoded['is_error'] == False
  ...
  def test_GetUserWithInvalidID(self):
      r = requests.get(self.baseUrl + self.userUrl + '/2', headers = self.headers, verify = False)
      decoded = r.json()
      assert decoded['is_error'] == True
      assert decoded['data'] == None
  ...
  def test_PutUserWithValidIDUpdateShouldEmail(self):
      ...
      r = requests.put(self.baseUrl + self.userUrl +'/1', data = json.dumps(update), headers = self.headers, verify = False)
      decoded = r.json()
      assert decoded['is_error'] == False
  ...
  def test_PutUserWithInvalidID(self):
      update = {'should_email': 'True'}
      r = requests.put(self.baseUrl + self.userUrl +'/2', data = json.dumps(update), headers = self.headers, verify = False)
      decoded = r.json()
      assert decoded['is_error'] == True
      assert decoded['data'] == None
      assert decoded['error_msg'] == 'User not found'
  ...
  def test_PutUserNotAsAdmin(self):
      ...
      r = requests.post(self.baseUrl + self.loginUrl, data = json.dumps(validUser), headers = self.headers, verify = False)
      nonAdminHeader = {'Accept': 'application/json', "Authorization": "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIyIiwidXNlcl9pZCI6MiwicGVybWlzc2lvbiI6MH0.5Br_NG0u_Kg2u11xyKqTGZAifyFHd19ca_HRRhu4j9Q", "Content-Type": "application/json" }
      r = requests.put(self.baseUrl + self.userUrl + '/1', data = json.dumps(update), headers = nonAdminHeader, verify = False)
      decoded = r.json()
      assert decoded['is_error'] == True
      assert decoded['data'] == None
      assert decoded['error_msg'] == 'You are not authorized'
  ...
  def test_DeleteUserWithValidID(self):
      validUser = {'email':'user13@admin.com', 'username':'someusername', 'password':'something'}
      r = requests.post(self.baseUrl + self.registerUrl, data = json.dumps(validUser), headers = self.headers, verify = False)
      r = requests.delete(self.baseUrl + self.userUrl + '/2', headers = self.headers, verify = False)
      decoded = r.json()
      assert decoded['is_error'] == False
      assert decoded['data'] == None

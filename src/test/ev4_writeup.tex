...
\begin{enumerate}
    \item In displaying the resource hierarchy, we display the existance of resources that the user does not have permission to see. We do this because otherwise there will be gaps in the resource hierarchy. However, these resources that the user does not have permission to see are displayed as "Mystery nodes" which are a different color and do not display any data about the resource other than its existence and placement in the resource hierarchy.
    \item When deleting a resource, the resource's children (if any) will become children of the resource's parent (or become a root if the delete resource had no parent resource).
    \item Deletion of a parent-child link in the resource hierarchy page causes the child (and its children) to become it's own tree with the child as the root.
\end{enumerate}
...
\begin{enumerate}
    ...
    \end{enumerate}
    \item Resource Hierarchy
    \begin{enumerate}
        \item Delete resources
        \item Delete parent child relationships
        \item Drag and drop resources to create new parent child relationships
        \item Users without view permissions will see mystery nodes
    \end{enumerate}
    \item New resource
    \begin{enumerate}

...
class AuthorizationTestCases(unittest.TestCase):
  ...
  def test_GetResourceWithValidID(self):
      validResource = {'name':'some resource', 'description':'some resource description', 'tags':['tag1', 'tag2']}
      requests.post(self.baseUrl + self.resourceUrl, data = json.dumps(validResource), headers = self.headers)
      r = requests.get(self.baseUrl + self.resourceUrl + '/1', headers = self.headers)
      decoded = r.json()
      assert decoded['is_error'] == False
      assert decoded['data'] == {u'description': u'some resource description', u'tags': ['tag1', 'tag2'], u'name': u'some resource', u'resource_id': 1}
  ...
  def test_GetResourcesWithValidQuery(self):
      ...
      requests.post(self.baseUrl + self.resourceUrl, data = json.dumps(validResource1), headers = self.headers)
      requests.post(self.baseUrl + self.resourceUrl, data = json.dumps(validResource2), headers = self.headers)
      requests.post(self.baseUrl + self.resourceUrl, data = json.dumps(validResource3), headers = self.headers)
      queryUrl = '/?required_tags=tag2,tag1&excluded_tags=tag3'
      r = requests.get(self.baseUrl + self.resourceUrl + queryUrl, headers = self.headers)
      decoded = r.json()
      assert decoded['is_error'] == False
  ...
  def test_PutResourceWithValidIDAllFieldsUpdated(self):
      ...
      putUrl = '/1'
      r = requests.put(self.baseUrl + self.resourceUrl + putUrl, data = json.dumps(update), headers = self.headers)
      decoded = r.json()
      assert decoded['is_error'] == False

...
We did run into an issue involving the Jackson JSON parser which is included with Java Spring. Jackson throws difficult-to-handle exceptions when the type of the JSON request values do not match with the expected types. Therefore, we had to circumvent the issue by performing type-checking on the front-end within ReactJS.
